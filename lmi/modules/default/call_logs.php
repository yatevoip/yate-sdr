...
function call_logs()
{
	...
	if (!is_file($yate_cdr)) {
		...
		elseif (isset($conf->structure["general"]["file"]) && $conf->structure["general"]["file"] != $yate_cdr) 
			box_note("Detected differences between the CDR file settings. "
			...
			box_note("There aren't any call logs.");

	} else {
	
		$ext = explode(".",$yate_cdr);
		if ($ext[1] != "csv") {
			box_note("Call logs must be in CSV format. Please modify the filename in 'defaults.php'.");
			return;
		}

		if (!is_readable($yate_cdr)) {
			box_note("Don't have permission to read the call logs file: '".$yate_cdr."'. Please run this command as root: 'chmod -R a+r ".$yate_cdr."'");
			return;
		}
